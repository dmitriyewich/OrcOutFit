...
- Коррекция позы оружия в руках (Held) через пресет `OrcOutFit\Weapons\<dff>.ini` (см. wiki «Оружие»); точка спавна FX выстрела (`muzzlePosn`: дым, гильзы и др.) берётся из той же Held-позы, **направление пули** (`origin`) — по ваниле. Вспышка у ствола (`gunflash` в DFF): при замене модели в руках узел синхронизируется с текущим клумпом; Held **не** смещает атомики под `gunflash`; на **теле** меш вспышки на кобуре не рисуется постоянно (см. wiki).
