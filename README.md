...
- Опционально: кастомные **mono PCM16 WAV** для заменённого оружия (`[Features] CustomWeaponSounds=1`: выстрел, distant, reload, dryfire, loop-оружие и др. — см. wiki «Оружие»); рядом с `OrcOutFit.asi` нужен **OpenAL32.dll** (в релизе CI кладётся в тот же каталог, что и ASI).
