...
- Коррекция позы оружия в руках (Held) через пресет `OrcOutFit\Weapons\<dff>.ini` (см. wiki «Оружие»); точка спавна FX выстрела (`muzzlePosn`: дым, гильзы и др.) берётся из той же Held-позы, **направление пули** (`origin`) — по ваниле. Вспышка у ствола (`gunflash` в DFF): при замене модели в руках узел синхронизируется с клумпом видимого меша (клон замены), а не с параллельным стоком в слоте, если слот временно указывает на сток или в слотовом клумпе нет dummy `gunflash` (кастомный DFF без узла — правка модели); на атомики под dummy `gunflash` **не** крутится тот же `OrcTryApplyHeldPoseOneFrame`, что на меш (локальные LTMs другие — ломает DoGunFlash); вместо этого кадр `gunflash` **сдвигается на мировую дельту** точки `m_vecFireOffset` (Held − ванильная кость R_Hand), как у `muzzlePosn`, **в том же клумпе**, что выбран для dummy `gunflash` (резолвер слот/клон, при отсутствии dummy — слот, клон замены или активный repl по цепочке; см. wiki). На **теле** меш вспышки на кобуре не рисуется постоянно (см. wiki).
- Замена моделей и текстур оружия (включая сценарии для SA:MP).
