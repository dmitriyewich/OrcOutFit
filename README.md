...
- Кастомные, стандартные и random-скины; random-пулы поддерживают случайный и последовательный режим выбора, а `SkinRandomIncludeVanilla=1` добавляет исходный ped как отдельный вариант. Кастомные скины по умолчанию применяются как native replacement к текущему ped без добавления gameplay skin ID, overlay оставлен как legacy-режим. Штатный `CPed::SetModelIndex` при создании/стриминге ped перехватывается до создания clump; для non-local/random ped поздний frame update не вызывает повторный `SetModelIndex`, поэтому если custom не был выбран в первичном spawn/model-set пути, ped остаётся vanilla до следующего естественного респавна/смены модели. Random-пулы стандартных ped привязаны к DFF-имени из `LoadPedObject` (`TRUTH`, `BMYDRUG`, `lapd1` и т.п.).
