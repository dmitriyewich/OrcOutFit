...
- Коррекция позы оружия в руках (Held) через пресет `OrcOutFit\Weapons\<dff>.ini` (см. wiki «Оружие»); точка спавна FX выстрела (`muzzlePosn`: дым, гильзы и др.) берётся из той же Held-позы, **направление пули** (`origin`) — по ваниле. Вспышка у ствола (`gunflash` в DFF): при замене модели в руках узел синхронизируется с текущим клумпом; на атомики под dummy `gunflash` **не** крутится тот же `OrcTryApplyHeldPoseOneFrame`, что на меш (локальные LTMs другие — ломает DoGunFlash); вместо этого кадр `gunflash` **сдвигается на мировую дельту** точки `m_vecFireOffset` (Held − ванильная кость R_Hand), как у `muzzlePosn`. На **теле** меш вспышки на кобуре не рисуется постоянно (см. wiki).
