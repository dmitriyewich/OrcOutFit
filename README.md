...
- Основное окно ImGui принудительно удерживается внутри клиентской области окна игры: позиция и размер хранятся в UI-состоянии, зажимаются до `Begin()` и применяются через `SetNextWindowPos/Size` при drag, resize и смене разрешения.
