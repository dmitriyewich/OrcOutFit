...
static int GetPreviewBasePedModelId(int source, int requestedModelId) {
    if (source == SKIN_PREVIEW_STANDARD || source == SKIN_PREVIEW_RANDOM) {
        if (OrcIsValidStandardSkinModel(requestedModelId))
            return requestedModelId;
    }
    const int localModelId = OrcGetLocalPlayerModelId();
    if (OrcIsValidStandardSkinModel(localModelId))
        return localModelId;
    if (OrcIsValidStandardSkinModel(g_standardSkinSelectedModelId))
...
static void RenderStandardSkinPreviewRequest() {
    ...
    if (RwCameraBeginUpdate(g_standardSkinPreview.camera)) {
        ...
        int v = rwCULLMODECULLBACK;
        RwRenderStateSet(rwRENDERSTATECULLMODE, reinterpret_cast<void*>(v));
        v = TRUE;
        RwRenderStateSet(rwRENDERSTATEZTESTENABLE, reinterpret_cast<void*>(v));
        v = TRUE;
        RwRenderStateSet(rwRENDERSTATEZWRITEENABLE, reinterpret_cast<void*>(v));
        v = rwSHADEMODEGOURAUD;
        RwRenderStateSet(rwRENDERSTATESHADEMODE, reinterpret_cast<void*>(v));
        v = FALSE;
        RwRenderStateSet(rwRENDERSTATEFOGENABLE, reinterpret_cast<void*>(v));
