...
static int OrcCachedWeaponModelIdForType(int wt, bool secondary) {
    const std::vector<int>& modelIds = secondary ? g_weaponModelId2 : g_weaponModelId;
    return wt > 0 && wt < static_cast<int>(modelIds.size()) ? modelIds[wt] : 0;
}

static bool OrcCachedWeaponTypeMatchesModelId(int wt, int modelId) {
    return modelId > 0 &&
        (OrcCachedWeaponModelIdForType(wt, false) == modelId ||
            OrcCachedWeaponModelIdForType(wt, true) == modelId);
}
...
static RpAtomic* OrcHeldPickFirstAtomicCb(RpAtomic* atomic, void* data) {
    auto** out = reinterpret_cast<RpAtomic**>(data);
    if (atomic && out && !*out)
        *out = atomic;
    return atomic;
}

static RpAtomic* OrcHeldGetFirstAtomicFromRwObject(RwObject* obj) {
    if (!obj)
        return nullptr;
    if (obj->type == rpATOMIC)
        return reinterpret_cast<RpAtomic*>(obj);
    if (obj->type == rpCLUMP) {
        RpAtomic* first = nullptr;
        RpClumpForAllAtomics(reinterpret_cast<RpClump*>(obj), OrcHeldPickFirstAtomicCb, &first);
        return first;
    }
    return nullptr;
}
...
static bool OrcHeldReplacementHasStockAnchor(int pedRef, const HeldWeaponReplacementState& st);
static bool OrcHeldReplacementIsActiveVisualForPed(CPed* ped, int wtHint, const HeldWeaponReplacementState& st);
...
static RwObject* OrcResolveActiveReplacementWeaponObject(CPed* ped) {
    ...
    if (it != g_heldWeaponReplacements.end()) {
        ...
        if (st.rwObject && st.rwObject->type == rpCLUMP &&
            OrcHeldReplacementIsActiveVisualForPed(ped, st.weaponType, st))
            return st.rwObject;
    }
    CPlayerPed* fp = FindPlayerPed(0);
    ...
    for (const auto& kv : g_heldWeaponReplacements) {
        ...
        if (st.rwObject && st.rwObject->type == rpCLUMP &&
            OrcHeldReplacementIsActiveVisualForPed(other, st.weaponType, st))
            return st.rwObject;
    }
    return nullptr;
}
...
static RwObject* OrcResolveHeldWeaponRwObject(CPed* ped) {
    ...
    if (ped->m_pWeaponObject) {
        if (!OrcHeldIsAnyReplacementCloneObject(ped->m_pWeaponObject))
            return ped->m_pWeaponObject;
        const int pedRef = CPools::GetPedRef(ped);
        auto it = (pedRef > 0) ? g_heldWeaponReplacements.find(pedRef) : g_heldWeaponReplacements.end();
        if (it != g_heldWeaponReplacements.end() &&
            (it->second.rwObject == ped->m_pWeaponObject || it->second.rwObjectSecondary == ped->m_pWeaponObject) &&
            OrcHeldReplacementIsActiveVisualForPed(ped, it->second.weaponType, it->second))
            return ped->m_pWeaponObject;
        return nullptr;
    }
    const int pedRef = CPools::GetPedRef(ped);
...
RpClump* OrcPedResolveGunflashTargetClump(CPed* ped, int wtHint) {
    ...
    auto pickReplacementClone = [&](const HeldWeaponReplacementState& st, CPed* choicePed) -> RpClump* {
        if (!st.rwObject || st.rwObject->type != rpCLUMP)
            return nullptr;
        CPed* ownerPed = choicePed ? choicePed : ped;
        const int wt = wtHint > 0 ? wtHint : st.weaponType;
        if (!OrcHeldReplacementIsActiveVisualForPed(ownerPed, wt, st))
            return nullptr;
...
static bool OrcHeldIsValidStockAnchorObject(RwObject* stock, const HeldWeaponReplacementState& st) {
    return stock && stock != st.rwObject && !OrcHeldIsAnyReplacementCloneObject(stock);
}

static bool OrcHeldReplacementHasStockAnchor(int pedRef, const HeldWeaponReplacementState& st) {
    if (pedRef <= 0 || !st.rwObject)
        return false;
    if (OrcHeldIsValidStockAnchorObject(st.originalObject, st))
        return true;
    auto d = g_deferredHeldWeaponStockRestore.find(pedRef);
    return d != g_deferredHeldWeaponStockRestore.end() &&
        d->second.clone == st.rwObject &&
        OrcHeldIsValidStockAnchorObject(d->second.stock, st);
}
...
static bool OrcHeldReplacementIsActiveVisualForPed(CPed* ped, int wtHint, const HeldWeaponReplacementState& st) {
    if (!ped || !st.rwObject)
        return false;
    const int pedRef = CPools::GetPedRef(ped);
    if (!OrcHeldReplacementHasStockAnchor(pedRef, st))
        return false;
    const int wt = wtHint > 0 ? wtHint : st.weaponType;
    return OrcHeldReplacementStateMatchesResolvedChoice(ped, wt, st);
}
...
static int GetPedCurrentWeaponType(CPed* ped) {
    ...
    if (wt <= 0)
        return 0;
    const bool needsAmmo = slot >= 2 && slot <= 9;
...
static int WeaponTypeFromModelId(int modelId) {
    return OrcWeaponsFindCachedTypeByModelId(modelId);
}
...
int OrcResolveWeaponHeldVisualWeaponType(CPed* ped) {
    ...
    for (int s = 0; s < 13; ++s) {
        ...
        if (t <= 0)
            continue;
        if (OrcCachedWeaponTypeMatchesModelId(t, mid))
            return t;
    }
...
static RwObject* CloneHeldWeaponForReplacementDraw(WeaponReplacementAsset& asset) {
    static_assert(OrcSelectWeaponCloneRenderContract(OrcWeaponCloneUsage::HeldReplacement) ==
        OrcWeaponCloneRenderContract::DefaultAtomicCallback);
    RwObject* clone = OrcCloneWeaponReplacementObject(asset);
    // `OrcCloneWeaponReplacementObject` intentionally leaves every atomic on
    // AtomicDefaultRenderCallBack. The held clone is drawn from inside the stock RenderWeaponCB;
    // assigning RenderWeaponCB to the nested clone re-enters weapon visibility logic and is unsafe
    // for arbitrary Guns DFF hierarchies. Body attachments use the separate prepared contract.
    if (!clone || !OrcNormalizeWeaponObjectToClumpForDefaultRender(clone)) {
        OrcDestroyRwObjectInstance(clone);
        return nullptr;
    }
    return clone;
}
...
int OrcResolveActiveHeldWeaponTypeForBodySuppression(CPed* ped) {
    ...
    const int selectedWt = GetPedSelectedWeaponTypeForReplace(ped);
    const int currentWt = GetPedCurrentWeaponType(ped);
    const bool hasHeldStockObject = ped->m_pWeaponObject && !OrcHeldIsAnyReplacementCloneObject(ped->m_pWeaponObject);
    if (selectedWt <= 0 && currentWt <= 0 && !hasHeldStockObject)
        return 0;

    const int visualWt = OrcResolveWeaponHeldVisualWeaponType(ped);
    if (visualWt > 0)
        return visualWt;
    if (selectedWt > 0)
        return selectedWt;
...
bool OrcPedWantsDualWieldHeld(CPed* ped, int wt) {
    ...
    if (OrcResolveWeaponHeldVisualWeaponType(ped) != wt)
        return false;
    if (!OrcWeaponTypeCanUseVanillaInfoContract(wt, static_cast<int>(WEAPONTYPE_PARACHUTE)))
        return false;
...
/// Первый атом clump, не привязанный к dummy gunflash (mesh оружия, не FX).
static RpAtomic* OrcHeldGetReplClumpDrawAtomic(RwObject* obj) {
    if (!obj)
        return nullptr;
    if (obj->type == rpATOMIC)
        return reinterpret_cast<RpAtomic*>(obj);
    ...
    RwFrame* gunf = CClumpModelInfo::GetFrameFromName(clump, "gunflash");
    struct PickCtx {
        RwFrame* gunflashRoot;
        RpAtomic* chosen;
    } ctx{ gunf, nullptr };
    auto cb = [](RpAtomic* atomic, void* data) -> RpAtomic* {
        auto* c = reinterpret_cast<PickCtx*>(data);
        if (!atomic || c->chosen)
            return atomic;
        RwFrame* af = RpAtomicGetFrame(atomic);
        if (c->gunflashRoot && af && OrcHeldAtomicFrameIsUnderGunflashDummy(af, c->gunflashRoot))
            return atomic;
        c->chosen = atomic;
        return atomic;
    };
    RpClumpForAllAtomics(clump, cb, &ctx);
    if (ctx.chosen)
        return ctx.chosen;
    return OrcHeldGetFirstAtomicFromRwObject(obj);
}
...
static bool s_heldRwcbExtensionDisabledAfterSeh = false;
struct HeldReplDrawFrameState {
    unsigned ms = 0;
    RpAtomic* drewPrimaryAtomic = nullptr;
    RwObject* drewPrimaryStock = nullptr;
    RpAtomic* drewSecondaryAtomic = nullptr;
    RwObject* drewSecondaryStock = nullptr;
};
...
static RpAtomic* OrcHeldResolveLeftHandSecStockAtomic(CPed* ped, int wt, RwObject* stockWo, RpAtomic* priStock, RpAtomic* secStock);
...
static int OrcHeldRwcbExtensionSehFilter(EXCEPTION_POINTERS* exceptionInfo, int phase, RpAtomic* atomic) {
    s_heldRwcbExtensionDisabledAfterSeh = true;
    ...
    const uintptr_t rva = module && address
        ...
        : 0u;
    OrcLogError(
        "RenderWeaponCB_Detour: held extension disabled after SEH phase=%d ex=0x%08X address=%p module=%p rva=0x%08X atomic=%p",
        phase,
        code,
        address,
        module,
...
struct OrcFindAtomicInClumpCtx {
    RpAtomic* target = nullptr;
    bool found = false;
};

static RpAtomic* OrcFindAtomicInClumpCb(RpAtomic* atomic, void* data) {
    auto* ctx = reinterpret_cast<OrcFindAtomicInClumpCtx*>(data);
    if (ctx && atomic == ctx->target)
        ctx->found = true;
    return atomic;
}

static bool OrcAtomicIsDirectMemberOfClump(RpAtomic* atomic, RpClump* clump) {
    if (!atomic || !clump)
        return false;
    OrcFindAtomicInClumpCtx ctx{};
    ctx.target = atomic;
    RpClumpForAllAtomics(clump, OrcFindAtomicInClumpCb, &ctx);
    return ctx.found;
}
...
static bool OrcAtomicBelongsToClump(RpAtomic* atomic, RpClump* clump) {
    if (!atomic || !clump)
        return false;
    if (OrcAtomicIsDirectMemberOfClump(atomic, clump))
        return true;
    RwFrame* root = RpClumpGetFrame(clump);
    RwFrame* af = RpAtomicGetFrame(atomic);
    if (!root || !af)
...
static RwObject* OrcHeldGetReplacementStockObject(int pedRef, const HeldWeaponReplacementState& st) {
    ...
    if (pedRef <= 0 || !st.rwObject)
        return nullptr;
    auto d = g_deferredHeldWeaponStockRestore.find(pedRef);
    if (d == g_deferredHeldWeaponStockRestore.end() || d->second.clone != st.rwObject ||
        !OrcHeldIsValidStockAnchorObject(d->second.stock, st))
        return nullptr;
    return d->second.stock;
}

static RwObject* OrcHeldResolveStockObjectBeforeCloneDestroy(
    int pedRef, CPed* ped, const HeldWeaponReplacementState& state) {
    if (ped && OrcHeldIsValidStockAnchorObject(ped->m_pWeaponObject, state))
        return ped->m_pWeaponObject;
    return OrcHeldGetReplacementStockObject(pedRef, state);
}

static RwFrame* OrcHeldResolveRenderableGunflashFrame(RwObject* object) {
    if (!object || object->type != rpCLUMP)
        return nullptr;
    RwFrame* frame = CClumpModelInfo::GetFrameFromName(reinterpret_cast<RpClump*>(object), "gunflash");
    if (!frame || rwLinkListEmpty(&frame->objectList))
        return nullptr;
    return frame;
}

static void OrcHeldDetachGunflashBeforeCloneDestroy(
    CPed* ped, RwObject* clone, RwObject* stock, const char* reason) {
    if (!ped || !clone || !ped->m_pGunflashObject)
        return;

    RwFrame* const oldFrame = ped->m_pGunflashObject;
    bool frameBelongsToClone = false;
    RwFrame* stockFrame = nullptr;
    __try {
        RwFrame* const cloneRoot = GetRwObjectRootFrame(clone);
        if (!cloneRoot) {
            ped->m_pGunflashObject = nullptr;
            OrcLogError("held wr: gunflash cleared before rootless clone destroy reason=%s pedRef=%d clone=%p oldGf=%p",
                reason ? reason : "?", CPools::GetPedRef(ped), clone, oldFrame);
            return;
        }
        frameBelongsToClone = OrcRwFrameHasAncestor(oldFrame, cloneRoot);
        if (frameBelongsToClone && stock && stock != clone)
            stockFrame = OrcHeldResolveRenderableGunflashFrame(stock);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        ped->m_pGunflashObject = nullptr;
        OrcLogError("held wr: gunflash detach SEH ex=0x%08X reason=%s pedRef=%d clone=%p oldGf=%p",
            GetExceptionCode(), reason ? reason : "?", CPools::GetPedRef(ped), clone, oldFrame);
        return;
    }

    const OrcGunflashFrameBeforeCloneDestroy action = OrcSelectGunflashFrameBeforeCloneDestroy(
        frameBelongsToClone, stockFrame != nullptr);
    if (action == OrcGunflashFrameBeforeCloneDestroy::KeepCurrent)
        return;

    ped->m_pGunflashObject =
        action == OrcGunflashFrameBeforeCloneDestroy::UseStock ? stockFrame : nullptr;
    if (g_orcLogLevel >= OrcLogLevel::Info) {
        OrcLogInfoThrottled(942u, 1200u,
            "held wr: gunflash detached before clone destroy reason=%s pedRef=%d clone=%p oldGf=%p stock=%p newGf=%p",
            reason ? reason : "?", CPools::GetPedRef(ped), clone, oldFrame, stock, ped->m_pGunflashObject);
    }
}
...
static void OrcClearHeldWeaponReplacementStateForPed(int pedRef, int wt, const char* reason) {
    ...
    CPed* const ped = OrcSafeGetPed(pedRef);
    RwObject* const stock = OrcHeldResolveStockObjectBeforeCloneDestroy(pedRef, ped, st);
    OrcHeldDetachGunflashBeforeCloneDestroy(ped, st.rwObject, stock, reason);
    OrcHeldDetachGunflashBeforeCloneDestroy(ped, st.rwObjectSecondary, stock, reason);
    g_deferredHeldWeaponStockRestore.erase(pedRef);
...
static bool OrcHeldAtomicMatchesWeaponModelTemplate(RpAtomic* atomic, int wt) {
    if (!atomic || wt <= 0 || !atomic->geometry)
        return false;
    const int modelIds[2] = {
        OrcCachedWeaponModelIdForType(wt, false),
        OrcCachedWeaponModelIdForType(wt, true),
    };
    for (int modelIndex = 0; modelIndex < 2; ++modelIndex) {
        const int modelId = modelIds[modelIndex];
        if (modelId <= 0 || (modelIndex == 1 && modelId == modelIds[0]))
            continue;
        CBaseModelInfo* mi = CModelInfo::GetModelInfo(modelId);
        if (mi && mi->m_pRwObject && OrcAtomicSharesGeometryWithWeaponObject(atomic, mi->m_pRwObject))
            return true;
    }
    return false;
}
...
/// Оставляет ориентацию clone root (из stock LTM), подменяет только world-pos кости руки.
static void OrcHeldGraftBonePosOntoCloneRoot(CPed* ped, int boneId, RwObject* clone) {
    if (!ped || !clone)
        return;
    ...
    if (!bone || !df)
        return;
    __try {
        RwMatrix* m = RwFrameGetMatrix(df);
        m->pos = bone->pos;
        RwMatrixUpdate(m);
        RwFrameUpdateObjects(df);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        OrcLogError("OrcHeldGraftBonePosOntoCloneRoot: SEH ex=0x%08X", GetExceptionCode());
    }
}
...
static void OrcHeldAlignReplGripOnWorldMatrix(CPed* ped, RwMatrix& m, bool leftHand) {
    if (!ped || !OrcRwMatrixFinite(&m) || !OrcRwMatrixAxesNonDegenerate(&m))
        return;
    RwMatrix* hand = OrcGetBoneMatrix(ped, leftHand ? BONE_L_HAND : BONE_R_HAND);
    if (!hand)
        return;
    float dotAt = m.at.x * hand->at.x + m.at.y * hand->at.y + m.at.z * hand->at.z;
    if (dotAt < 0.0f) {
        m.at.x = -m.at.x;
        m.at.y = -m.at.y;
        m.at.z = -m.at.z;
    ...
    const float dotUp = m.up.x * hand->up.x + m.up.y * hand->up.y + m.up.z * hand->up.z;
    if (dotUp < 0.0f) {
        m.up.x = -m.up.x;
        m.up.y = -m.up.y;
        m.up.z = -m.up.z;
...
/// Repl-clone: выравнивание grip к кости руки (явный flip осей; RwMatrixRotate PRECONCAT не менял dot в логе).
static void OrcHeldAlignReplCloneGripToHand(CPed* ped, RwObject* clone, bool leftHand) {
    if (!ped || !clone || !HeldWeaponRwObjectIsReplacementClone(ped, clone))
        return;
    RwMatrix* hand = OrcGetBoneMatrix(ped, leftHand ? BONE_L_HAND : BONE_R_HAND);
    RwFrame* df = GetRwObjectRootFrame(clone);
    RwMatrix* m = df ? RwFrameGetMatrix(df) : nullptr;
    if (!hand || !m)
        return;
    __try {
        float dotAt = m->at.x * hand->at.x + m->at.y * hand->at.y + m->at.z * hand->at.z;
        ...
        float dotUp = m->up.x * hand->up.x + m->up.y * hand->up.y + m->up.z * hand->up.z;
        if (dotUp < 0.0f) {
            m->up.x = -m->up.x;
            m->up.y = -m->up.y;
            m->up.z = -m->up.z;
        ...
        OrcLogError("OrcHeldAlignReplCloneGripToHand: SEH ex=0x%08X", GetExceptionCode());
    }
}
...
/// Тот же пайплайн, что primary draw-swap: stock LTM + grip-fix repl-DFF + pivot к R_HAND.
static void OrcHeldSyncPrimaryCloneMatrixFromStock(CPed* ped, RpAtomic* priStock, RwObject* priClone) {
    if (!ped || !priStock || !priClone)
        return;
    CopyStockAtomicLtmToCloneRoot(priStock, priClone);
    if (!HeldWeaponRwObjectIsReplacementClone(ped, priClone))
        return;
    RwFrame* dfGrip = GetRwObjectRootFrame(priClone);
    RwMatrix* mGrip = dfGrip ? RwFrameGetMatrix(dfGrip) : nullptr;
    if (mGrip) {
        OrcHeldAlignReplGripOnWorldMatrix(ped, *mGrip, false);
        RwMatrixUpdate(mGrip);
        RwFrameUpdateObjects(dfGrip);
    }
    CVector priAp{};
    if (!OrcTryGetAtomicWorldPos(priStock, priAp))
        return;
    RwMatrix* rHand = OrcGetBoneMatrix(ped, BONE_R_HAND);
    RwFrame* df = GetRwObjectRootFrame(priClone);
    RwMatrix* m = df ? RwFrameGetMatrix(df) : nullptr;
    if (!rHand || !m || !OrcAtomicWorldPosUsable(priAp))
        return;
    __try {
        m->pos.x += rHand->pos.x - priAp.x;
        m->pos.y += rHand->pos.y - priAp.y;
        m->pos.z += rHand->pos.z - priAp.z;
        RwMatrixUpdate(m);
        RwFrameUpdateObjects(df);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        OrcLogError("OrcHeldSyncPrimaryCloneMatrixFromStock: SEH ex=0x%08X", GetExceptionCode());
    }
}

static void OrcHeldSetRwMatrixIdentity(RwMatrix* m) {
    if (!m)
        return;
    m->right.x = 1.0f;
    m->right.y = 0.0f;
    ...
    m->pos.y = 0.0f;
    m->pos.z = 0.0f;
    m->flags = static_cast<RwInt32>(rwMATRIXTYPEORTHONORMAL);
}

/// Draw-атом repl-clump на root: world LTM = root (как stock atomic), без внутреннего поворота DFF.
static bool OrcHeldAttachReplDrawAtomicToClumpRoot(RwObject* clone) {
    if (!clone || clone->type != rpCLUMP)
        return false;
    RpClump* clump = reinterpret_cast<RpClump*>(clone);
    RpAtomic* at = OrcHeldGetReplClumpDrawAtomic(clone);
    if (!at)
        return false;
    RwFrame* root = RpClumpGetFrame(clump);
    RwFrame* af = RpAtomicGetFrame(at);
    if (!root || !af || af == root)
        return af == root;
    bool otherOnFrame = false;
    struct AttachScanCtx {
        RpAtomic* drawAt;
        RwFrame* oldFrame;
        bool otherOnFrame;
    } scanCtx{ at, af, false };
    auto scanCb = [](RpAtomic* a, void* data) -> RpAtomic* {
        auto* ctx = reinterpret_cast<AttachScanCtx*>(data);
        if (a && a != ctx->drawAt && RpAtomicGetFrame(a) == ctx->oldFrame)
            ctx->otherOnFrame = true;
        return a;
    };
    RpClumpForAllAtomics(clump, scanCb, &scanCtx);
    otherOnFrame = scanCtx.otherOnFrame;
    __try {
        RpAtomicSetFrame(at, root);
        if (!otherOnFrame)
            RwFrameDestroy(af);
        RwFrameUpdateObjects(root);
        return true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        OrcLogError("OrcHeldAttachReplDrawAtomicToClumpRoot: SEH ex=0x%08X", GetExceptionCode());
        return false;
    }
}

/// На fire-анимации lock по bias может отдать «левый» атом, который ближе к R_HAND — берём twin-пару.
static RpAtomic* OrcHeldResolveLeftHandSecStockAtomic(CPed* ped, int wt, RwObject* stockWo, RpAtomic* priStock, RpAtomic* secStock) {
    if (!ped || !secStock)
        return secStock;
    CVector ap{};
    if (!OrcTryGetAtomicWorldPos(secStock, ap))
        return secStock;
    RwMatrix* lHand = OrcGetBoneMatrix(ped, BONE_L_HAND);
    RwMatrix* rHand = OrcGetBoneMatrix(ped, BONE_R_HAND);
    if (!lHand || !rHand)
        return secStock;
    if (OrcHeldDistSqToBonePos(ap, *lHand) + 0.04f < OrcHeldDistSqToBonePos(ap, *rHand))
        return secStock;
    if (stockWo && priStock && secStock != priStock) {
        RpAtomic* other = OrcHeldFindOtherWeaponAtomicOnStock(stockWo, priStock, ped, wt);
        if (other)
            return other;
    }
    return secStock;
...
/// Строго ванильная матрица левого ствола dual-wield.
/// Ванилла (RenderWeaponPedsForPC @0x732F30, GTA SA 1.0 US): берёт мировую матрицу кости
/// L Hand (bone 34), preconcat поворот 180° вокруг локальной оси X, preconcat сдвиг (0.04,-0.05,0),
/// и этим рисует тот же weapon-clump во второй раз. Здесь то же самое накладывается на root клона,
/// поэтому без поиска «левого» stock-атома, без pivot к кости и без twin-эвристик (они и давали
/// дубль правого ствола и «плавание» при ходьбе).
static void OrcHeldApplyVanillaLeftHandMatrixToCloneRoot(CPed* ped, RwObject* clone) {
    if (!ped || !clone)
        return;
    if (HeldWeaponRwObjectIsReplacementClone(ped, clone))
        OrcHeldAttachReplDrawAtomicToClumpRoot(clone);
    RwMatrix* lHand = OrcGetBoneMatrix(ped, BONE_L_HAND);
    RwFrame* df = GetRwObjectRootFrame(clone);
    RwMatrix* m = df ? RwFrameGetMatrix(df) : nullptr;
    if (!lHand || !m)
        return;
    __try {
        std::memcpy(m, lHand, sizeof(RwMatrix));
        RwV3d axisX{1.0f, 0.0f, 0.0f};
        RwMatrixRotate(m, &axisX, 180.0f, rwCOMBINEPRECONCAT);
        RwV3d ofs{0.04f, -0.05f, 0.0f};
        RwMatrixTranslate(m, &ofs, rwCOMBINEPRECONCAT);
        RwMatrixUpdate(m);
        RwFrameUpdateObjects(df);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        OrcLogError("OrcHeldApplyVanillaLeftHandMatrixToCloneRoot: SEH ex=0x%08X", GetExceptionCode());
    }
}

/// Левый clone2 dual-wield: строго ванильная матрица от кости L Hand (см. helper выше).
/// stock-атомы (priStock/secStock) для позиционирования не используются — только для триггера/дедупа
/// отрисовки на уровне вызова.
static void OrcHeldCopyDualSecondaryLtmToCloneRoot(CPed* ped,
    ...
    OrcHeldApplyVanillaLeftHandMatrixToCloneRoot(ped, clone);

    if (g_orcLogLevel >= OrcLogLevel::Info && g_heldWeaponTrace >= 1) {
        RwMatrix* lHand = OrcGetBoneMatrix(ped, BONE_L_HAND);
        RwFrame* df = GetRwObjectRootFrame(clone);
        const RwMatrix* m = df ? RwFrameGetMatrix(df) : nullptr;
        float dotAtHand = 0.0f;
        float dotUpHand = 0.0f;
        float dotRtHand = 0.0f;
        if (lHand && m) {
            dotAtHand = m->at.x * lHand->at.x + m->at.y * lHand->at.y + m->at.z * lHand->at.z;
            dotUpHand = m->up.x * lHand->up.x + m->up.y * lHand->up.y + m->up.z * lHand->up.z;
            dotRtHand = m->right.x * lHand->right.x + m->right.y * lHand->right.y + m->right.z * lHand->right.z;
        }
        OrcLogInfoThrottled(54, 3000u,
            "held wr: dual sec LTM mode=vanillaLHand pedRef=%d clone2=%p lHandPos=(%.2f,%.2f,%.2f) dotAtL=%.3f dotUpL=%.3f dotRtL=%.3f",
            CPools::GetPedRef(ped),
            clone,
            lHand ? lHand->pos.x : 0.0f,
            lHand ? lHand->pos.y : 0.0f,
            lHand ? lHand->pos.z : 0.0f,
            dotAtHand,
            dotUpHand,
            dotRtHand);
    }
...
static void OrcHeldCopyStockOrHandLtmToCloneRoot(CPed* ped,
    ...
    RwObject* clone,
    bool secondaryHand,
    RwObject* priCloneForTwin) {
    if (!stockAtomic || !clone)
        return;
    if (secondaryHand && ped && wt > 0 && OrcPedWantsDualWieldHeld(ped, wt)) {
        OrcHeldCopyDualSecondaryLtmToCloneRoot(ped, wt, priStock, stockAtomic, clone, priCloneForTwin);
        return;
    }
    OrcHeldSyncPrimaryCloneMatrixFromStock(ped, stockAtomic, clone);
    if (ped && HeldWeaponRwObjectIsReplacementClone(ped, clone)) {
        CVector priAp{};
        float dlR = 0.0f;
        if (OrcTryGetAtomicWorldPos(stockAtomic, priAp)) {
            RwMatrix* rHand = OrcGetBoneMatrix(ped, BONE_R_HAND);
            if (rHand && OrcAtomicWorldPosUsable(priAp))
                dlR = OrcHeldDistSqToBonePos(priAp, *rHand);
        }
        if (g_orcLogLevel >= OrcLogLevel::Info && g_heldWeaponTrace >= 1) {
            RwMatrix* rHand = OrcGetBoneMatrix(ped, BONE_R_HAND);
            RwFrame* df = GetRwObjectRootFrame(clone);
            ...
            float dotUpR = 0.0f;
            if (rHand && m) {
                dotAtR = m->at.x * rHand->at.x + m->at.y * rHand->at.y + m->at.z * rHand->at.z;
                dotUpR = m->up.x * rHand->up.x + m->up.y * rHand->up.y + m->up.z * rHand->up.z;
            }
            OrcLogInfoThrottled(55, 3000u,
                "held wr: dual pri LTM priPosGrip pedRef=%d priStock=%p clone=%p dlR=%.3f dotAtR=%.3f dotUpR=%.3f",
                CPools::GetPedRef(ped),
                stockAtomic,
                clone,
                dlR,
...
static bool OrcHeldInferSecondaryHandFromAtomic(CPed* ped, RpAtomic* atomic, int wt) {
    if (!ped || !atomic || wt <= 0)
        return false;
    const int mid1 = OrcCachedWeaponModelIdForType(wt, false);
    const int mid2 = OrcCachedWeaponModelIdForType(wt, true);
...
static void OrcHeldTraceLogRenderWeaponCb(RpAtomic* atomic, bool appliedHeldPose, bool rwcbMatchLocalPl) {
    ...
    RpGeometry* const traceGeometry = atomic->geometry;
    CPlayerPed* pl = FindPlayerPed(0);
    if (!pl) {
    ...
    OrcLogInfoThrottled(506, iv,
        "held hook RWCB(0x733670): atomic=%p frame=%p rdcb=%p atomicPipe=%p geometry=%p geomFlags=0x%08X atomicRepEntry=%p geomRepEntry=%p pedRef=%d skinIni=\"%s\" skinRaw=\"%s\" matchWo=%d "
        "wo=%p woT=%d repl=%d slot=%u slotWt=%d visWt=%d secondary=%d heldCfgEn=%d off=(%.3f,%.3f,%.3f) rotDeg=(%.1f,%.1f,%.1f) "
        "sc=%.3f appliedPose=%d",
        static_cast<void*>(atomic), static_cast<void*>(fr), rdcb, static_cast<void*>(atomic->pipeline),
        static_cast<void*>(traceGeometry), traceGeometry ? static_cast<unsigned>(traceGeometry->flags) : 0u,
        static_cast<void*>(atomic->repEntry), traceGeometry ? static_cast<void*>(traceGeometry->repEntry) : nullptr,
        pr, skinIni.c_str(), skinRaw.c_str(),
...
static bool OrcTryGetHeldReplacementDrawContextForPed(CPed* ped, RpAtomic* stockAtomic, HeldReplacementDrawContext& out) {
    if (!ped || !stockAtomic || OrcIsVanillaCustomWeaponDrawActive())
        return false;
...
static bool OrcRenderHeldReplacementCloneAtStockDraw(const HeldReplacementDrawContext& ctx, const char* phase) {
    ...
    if (drawFrame.ms != frameMs) {
        ...
        drawFrame.drewPrimaryStock = nullptr;
        drawFrame.drewSecondaryAtomic = nullptr;
        drawFrame.drewSecondaryStock = nullptr;
    }
    if (!ctx.secondaryHand) {
        if (ctx.stock && drawFrame.drewPrimaryStock == ctx.stock) {
            if (g_orcLogLevel >= OrcLogLevel::Info && g_heldWeaponTrace >= 1) {
                OrcLogInfoThrottled(54, 2500u,
                    "held wr: draw-swap suppress stock sibling phase=%s pedRef=%d wt=%d stock=%p stockAtomic=%p frameMs=%u",
                    phase ? phase : "?",
                    ctx.pedRef,
                    ctx.weaponType,
                    ctx.stock,
        ...
        if (ctx.stock && drawFrame.drewSecondaryStock == ctx.stock) {
            if (g_orcLogLevel >= OrcLogLevel::Info && g_heldWeaponTrace >= 1) {
                OrcLogInfoThrottled(55, 2500u,
                    "held wr: draw-swap suppress stock sibling phase=%s pedRef=%d wt=%d stock=%p stockAtomic=%p secondary=1 frameMs=%u",
                    phase ? phase : "?",
                    ctx.pedRef,
                    ctx.weaponType,
                    ctx.stock,
        ...
        if (drawFrame.drewSecondaryAtomic == ctx.stockAtomic) {
            if (g_orcLogLevel >= OrcLogLevel::Info && g_heldWeaponTrace >= 1) {
    ...
    if (ctx.stockAtomic)
        OrcHeldCopyStockOrHandLtmToCloneRoot(ctx.ped,
            ...
            drawClone,
            ctx.secondaryHand,
            ctx.state ? ctx.state->rwObject : nullptr);
    else if (ctx.stock)
        CopyStockHeldWeaponRwMatricesToClone(ctx.stock, drawClone, false);
    OrcHeldPoseInvalidateBaselineForRwObject(drawClone);
    const bool dualSecRepl = ctx.secondaryHand && OrcPedWantsDualWieldHeld(ctx.ped, ctx.weaponType) &&
        HeldWeaponRwObjectIsReplacementClone(ctx.ped, drawClone);
    if (!dualSecRepl) {
    ...
    WeaponTextureAsset* textureAsset = g_weaponTexturesEnabled
        ? OrcResolveUsableWeaponTextureAssetForPed(ctx.ped,
              ctx.weaponType,
              true,
              ctx.state->replacementKey.empty() ? nullptr : &ctx.state->replacementKey)
        : nullptr;
    const bool drew = OrcRenderWeaponObjectLikeVanillaWithTextures(
        ctx.ped, ctx.weaponType, drawClone, textureAsset, true, g_weaponTexturesEnabled);
...
static bool OrcTryDrawDualHeldReplacementViaRwcb(RpAtomic* atomic, HeldReplacementDrawContext& ctx) {
    ...
    RpAtomic* handRight = nullptr;
    RpAtomic* handLeft = nullptr;
    if (ctx.stock->type == rpCLUMP) {
        OrcHeldLockDualStockAtomicsByHands(ctx.ped, *ctx.state, ctx.stock);
        handRight = ctx.state->lastPrimaryStockAtomic;
        handLeft = ctx.state->lastSecondaryStockAtomic;
    }

    RpAtomic* rwcbForOrig = handRight;
    if (!rwcbForOrig || !OrcAtomicUsesRenderWeaponCb(rwcbForOrig)) {
        if (handLeft && OrcAtomicUsesRenderWeaponCb(handLeft))
            rwcbForOrig = handLeft;
        else if (OrcAtomicUsesRenderWeaponCb(atomic))
            rwcbForOrig = atomic;
    }
    if (!rwcbForOrig)
        return false;
    ...
    s_heldRenderWeaponCbAtomic = rwcbForOrig;
    OrcCallRenderWeaponCbOrigSafe(rwcbForOrig, "rwcb:dualOrig");
    ...
    // Сначала правый (основной) — пайплайн pri не менять. Потом левый от sec stock.
    if (handRight) {
        HeldReplacementDrawContext priCtx = ctx;
        priCtx.stockAtomic = handRight;
        priCtx.secondaryHand = false;
        OrcRenderHeldReplacementCloneAtStockDraw(priCtx, "repl:rwcbDualPri");
    }

    if (handLeft && handLeft != handRight) {
        if (ctx.stock->type == rpCLUMP) {
            OrcHeldLockDualStockAtomicsByHands(ctx.ped, *ctx.state, ctx.stock);
            handRight = ctx.state->lastPrimaryStockAtomic;
            handLeft = ctx.state->lastSecondaryStockAtomic;
        }
        handLeft = OrcHeldResolveLeftHandSecStockAtomic(ctx.ped, ctx.weaponType, ctx.stock, handRight, handLeft);
        if (handLeft) {
            ctx.state->lastSecondaryStockAtomic = handLeft;
            HeldReplacementDrawContext secCtx = ctx;
            secCtx.stockAtomic = handLeft;
            secCtx.secondaryHand = true;
            if (g_orcLogLevel >= OrcLogLevel::Info && g_heldWeaponTrace >= 1) {
                OrcLogInfoThrottled(53, 2000u,
                    "held wr: rwcbDualSec begin pedRef=%d wt=%d handL=%p handR=%p",
                    ctx.pedRef,
                    ctx.weaponType,
                    handLeft,
                    handRight);
            }
            OrcRenderHeldReplacementCloneAtStockDraw(secCtx, "repl:rwcbDualSec");
        }
    }
    return true;
}
...
static RpAtomic* __cdecl AtomicDefaultRenderCallBack_Detour(RpAtomic* atomic) {
    ...
    // Body atomics reach this callback directly for Orc replacements or from stock RenderWeaponCB. They are not
    // held candidates, so pass them to MinHook's original trampoline without suppress/pose/swap logic.
    if (OrcIsBodyAttachmentDrawActive() || OrcIsVanillaCustomWeaponDrawActive())
        return g_AtomicDefaultRender_Orig ? g_AtomicDefaultRender_Orig(atomic) : nullptr;
    HeldRwcbFrameRestore restore{};
    if (g_enabled) {
        const bool allPeds = g_renderAllPedsWeapons;
...
static void __cdecl RenderWeaponCB_Detour(RpAtomic* atomic) {
    ...
    const bool bodyAttachmentDraw = OrcIsBodyAttachmentDrawActive();
    if (OrcSelectWeaponAtomicDispatch(
            OrcIsVanillaCustomWeaponDrawActive(), bodyAttachmentDraw) ==
        OrcWeaponAtomicDispatch::VanillaWeaponCallback) {
        OrcCallRenderWeaponCbOrigSafe(
            atomic, bodyAttachmentDraw ? "rwcb:bodyStock" : "rwcb:customVanilla");
        return;
    }
    bool appliedPose = false;
    bool rwcbMatchLocal = false;
    HeldRwcbFrameRestore restore{};
    if (g_enabled && !s_heldRwcbExtensionDisabledAfterSeh) {
        ...
        if (localHeldGate || needRemote) {
            ...
            } __except (OrcHeldRwcbExtensionSehFilter(GetExceptionInformation(), sehPhase, atomic)) {
            }
        }
    }
    if (g_heldWeaponTrace >= 1 && g_orcLogLevel >= OrcLogLevel::Info)
...
static RwObject* OrcHeldResolveDualStockClumpForPed(CPed* ped, HeldWeaponReplacementState& st) {
    if (!ped)
        return nullptr;
    const int pedRef = CPools::GetPedRef(ped);
    RwObject* stockWo = OrcHeldGetReplacementStockObject(pedRef, st);
...
void OrcDestroyAllHeldWeaponReplacementInstances() {
    ...
    for (auto& kv : g_heldWeaponReplacements) {
        HeldWeaponReplacementState& st = kv.second;
        CPed* ped = OrcSafeGetPed(kv.first);
        RwObject* const stock = OrcHeldResolveStockObjectBeforeCloneDestroy(kv.first, ped, st);
        ...
        st.poseSynced = false;
        OrcHeldDetachGunflashBeforeCloneDestroy(ped, st.rwObject, stock, "destroyAll");
        OrcHeldDetachGunflashBeforeCloneDestroy(ped, st.rwObjectSecondary, stock, "destroyAll");
...
static void OrcHeldLockDualStockAtomicsByHands(CPed* ped, HeldWeaponReplacementState& st, RwObject* stockWo) {
    ...
    // Twin colt: правый проход — RenderWeaponCB; левый — второй атом. Не назначать pri по world-bias при biasSep≈0.
    if (iRwcb >= 0 && collect.n >= 2) {
...
static bool OrcHeldClassifyDualStockAtomicDraw(CPed* ped,
    ...
    if (OrcAtomicUsesRenderWeaponCb(stockAtomic)) {
        ...
        if (st.lastPrimaryStockAtomic && stockAtomic == st.lastPrimaryStockAtomic) {
            *outSecondaryHand = false;
            return true;
        }
        *outSecondaryHand = false;
...
void OrcWeaponSuppressBodyForHeldVisualWeapon(CPed* ped, std::vector<char>* suppress) {
    if (!ped || !suppress || !g_weaponReplacementEnabled || !g_weaponReplacementInHands)
        return;
    const int activeHeldWt = OrcResolveActiveHeldWeaponTypeForBodySuppression(ped);
    if (activeHeldWt <= 0)
        return;
    auto suppressWt = [&](int wt) { OrcMarkBodyWeaponTypeSuppressed(suppress, wt); };
    if (ped->m_nSelectedWepSlot < 13)
        suppressWt(static_cast<int>(ped->m_aWeapons[ped->m_nSelectedWepSlot].m_eWeaponType));
    suppressWt(GetPedCurrentWeaponType(ped));
    suppressWt(GetPedSelectedWeaponTypeForReplace(ped));
    suppressWt(WeaponTypeFromModelId(ped->m_nWeaponModelId));
    suppressWt(activeHeldWt);
}
...
static void OrcHeldEnsureSecondaryClone(CPed* ped, HeldWeaponReplacementState& state, WeaponReplacementAsset& asset) {
    ...
    if (!state.rwObjectSecondary) {
        state.rwObjectSecondary = CloneHeldWeaponForReplacementDraw(asset);
        if (state.rwObjectSecondary)
            OrcHeldAttachReplDrawAtomicToClumpRoot(state.rwObjectSecondary);
...
static void OrcHeldRenderDualSecondaryClone(CPed* ped, HeldWeaponReplacementState& state, const char* phase) {
    ...
    if (OrcHeldSecondaryAlreadyDrawnThisPass(pedRef))
        return;
    ...
    RwObject* stockWo = OrcHeldGetReplacementStockObject(pedRef, state);
    if (!stockWo) {
        if (g_orcLogLevel >= OrcLogLevel::Info && g_heldWeaponTrace >= 1) {
    ...
    if (!secStock || secStock == state.lastPrimaryStockAtomic) {
        secStock = OrcHeldFindOtherWeaponAtomicOnStock(stockWo, state.lastPrimaryStockAtomic, ped, state.weaponType);
        if (secStock)
    ...
    RwObject* drawStock = stockWo;
    if (drawStock && drawStock->type == rpCLUMP)
        OrcHeldLockDualStockAtomicsByHands(ped, state, drawStock);
    secStock = OrcHeldResolveLeftHandSecStockAtomic(ped, state.weaponType, drawStock, state.lastPrimaryStockAtomic, secStock);
    state.lastSecondaryStockAtomic = secStock;
    OrcHeldPumpSecondaryStockAtomicMatrix(secStock);
...
static void OrcHeldMarkWaitingForStockDraw(
    CPed* ped,
    int pedRef,
    int wt,
    ...
    if (state.rwObjectSecondary)
        OrcHeldPoseInvalidateBaselineForRwObject(state.rwObjectSecondary);
    OrcHeldReplacementInvalidateDrawStateForPed(pedRef);
    if (g_orcLogLevel >= OrcLogLevel::Info && g_heldWeaponTrace >= 1) {
        OrcLogInfoThrottled(35, 1200u,
            "held wr: waiting stock phase=%s pedRef=%d wt=%d key=%s slotWo=%p clone=%p",
            phase ? phase : "?",
            pedRef,
            wt,
            key.empty() ? "-" : key.c_str(),
            ped ? ped->m_pWeaponObject : nullptr,
            state.rwObject);
    }
}
...
static void OrcCaptureHeldReplacementForDrawSwap(
    ...
    if (!OrcHeldIsValidStockAnchorObject(stockForCopy, state)) {
        OrcHeldMarkWaitingForStockDraw(ped, pedRef, wt, key, state, phase);
        return;
    }

    state.originalObject = stockForCopy;
    if (state.rwObjectSecondary) {
        // PRO twin: root posecopy на clone1 ломает rwcbDualPri (clump root != stock-atomic LTM).
        state.poseSynced = false;
        OrcHeldPoseInvalidateBaselineForRwObject(state.rwObject);
        if (state.rwObjectSecondary)
            OrcHeldPoseInvalidateBaselineForRwObject(state.rwObjectSecondary);
...
void OrcPrepareHeldWeaponReplacementBefore(CPed* ped) {
    ...
    if (s_heldRwcbExtensionDisabledAfterSeh) {
        OrcClearHeldWeaponReplacementStateForPed(pedRefEarly, 0, "rwcbSehFailClosed");
        return;
    }
    ...
    if (GetPedSelectedWeaponTypeForReplace(ped) <= 0 && !ped->m_pWeaponObject && ped->m_nWeaponModelId <= 0) {
        OrcClearHeldWeaponReplacementStateForPed(pedRefEarly, wt, "noHeldStock");
        if (g_orcLogLevel >= OrcLogLevel::Info && g_heldWeaponTrace >= 1) {
            OrcLogInfoThrottled(56, 1200u,
                "held wr: ignore stale held wt pedRef=%d wtRes=%d savedWt=%d",
                pedRefEarly,
                wt,
                static_cast<int>(ped->m_nSavedWeapon));
        }
        return;
    }
    ...
    if (!state.rwObject || state.weaponType != wt || state.replacementKey != asset->key) {
        ...
        RwObject* const stock = OrcHeldResolveStockObjectBeforeCloneDestroy(pedRef, ped, state);
        OrcHeldDetachGunflashBeforeCloneDestroy(ped, state.rwObject, stock, "reclone");
        OrcHeldDetachGunflashBeforeCloneDestroy(ped, state.rwObjectSecondary, stock, "reclone");
        OrcDestroyRwObjectInstance(state.rwObject);
        OrcHeldDestroySecondaryClone(state);
        OrcHeldReplacementInvalidateDrawStateForPed(pedRef);
        state.poseSynced = false;
        const DWORD tClone0 = GetTickCount();
        state.rwObject = CloneHeldWeaponForReplacementDraw(*asset);
    ...
    if (state.rwObject && state.weaponType == wt && state.replacementKey == asset->key) {
        ...
        if (stockForCopy && ped->m_pWeaponObject == stockForCopy) {
            if (OrcHeldIsAnyReplacementCloneObject(stockForCopy)) {
                ...
                OrcHeldMarkWaitingForStockDraw(ped, pedRef, wt, asset->key, state, "swapStockBadSource");
                return;
            }
        ...
        if (stockForCopy && ped->m_pWeaponObject == state.rwObject) {
            if (OrcHeldIsAnyReplacementCloneObject(stockForCopy)) {
                OrcLogInfoThrottled(33, 2000u,
                    ...
                    pedRefEarly, wt, asset->key.c_str(), stockForCopy, state.rwObject);
                OrcHeldMarkWaitingForStockDraw(ped, pedRef, wt, asset->key, state, "reSyncBadSource");
                return;
            }
        ...
        if (state.hideBaseMode && state.originalObject && !ped->m_pWeaponObject) {
            OrcHeldMarkWaitingForStockDraw(ped, pedRef, wt, asset->key, state, "lostStockSlot");
            return;
        }
    }

    // SA:MP may leave `m_pWeaponObject` null during ped render while wt still resolves.
    // Do not draw the clone without a stock draw callback: it drifts from the hand and can duplicate.
    if (state.hideBaseMode && !state.originalObject && state.rwObject && state.weaponType == wt &&
        state.replacementKey == asset->key) {
        if (!ped->m_pWeaponObject) {
            OrcHeldMarkWaitingForStockDraw(ped, pedRef, wt, asset->key, state, "noStockSlot");
            return;
        }
        state.hideBaseMode = false;
    }
    ...
    if (!ped->m_pWeaponObject) {
        ...
        OrcHeldMarkWaitingForStockDraw(ped, pedRef, wt, asset->key, state, "nullSlot");
        return;
    }

    // Held and body replacement atomics stay on AtomicDefaultRenderCallBack. Body draws carry a dedicated
    // guard, so this held detour passes them straight to the original RW trampoline.
    ...
    if (!stockForCopy || stockForCopy == state.rwObject) {
        OrcLogInfoThrottled(31, 2000u,
            ...
            pedRefEarly, wt, asset->key.c_str(), stockForCopy, state.rwObject);
        OrcHeldMarkWaitingForStockDraw(ped, pedRef, wt, asset->key, state, "firstSwapInvalidSource");
        return;
    }
    if (OrcHeldIsAnyReplacementCloneObject(stockForCopy)) {
        OrcLogInfoThrottled(34, 2000u,
            ...
            pedRefEarly, wt, asset->key.c_str(), stockForCopy, state.rwObject);
        OrcHeldMarkWaitingForStockDraw(ped, pedRef, wt, asset->key, state, "firstSwapBadSource");
        return;
    }
...
static bool ModelIdMatchesWeaponType(int modelId, int wt) {
    return OrcCachedWeaponTypeMatchesModelId(wt, modelId);
}
...
static void __fastcall RemoveWeaponModel_Hook(CPed* ped, void* /*edx*/, int modelIndex) {
    ...
    st.originalObject = nullptr;
    OrcHeldDetachGunflashBeforeCloneDestroy(ped, st.rwObject, nullptr, "removeWeaponModel");
    OrcDestroyRwObjectInstance(st.rwObject);
    st.rwObject = nullptr;
    st.poseSynced = false;
    st.captureActive = false;
