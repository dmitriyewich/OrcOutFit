...
[*]Сохранение настроек выполняется пакетно: INI собирается в памяти и записывается одним проходом, чтобы игра не зависала от серии отдельных WinAPI INI-записей.
[*]Меню рисуется через D3D9 hooks (`Present` / fallback `EndScene`, `Reset`) после первого drawable-кадра игры. Одиночная игра не ждёт полной загрузки SA:MP; в неизвестных сборках SA:MP меню и курсор работают как в single-player fallback.
[*]ПКМ временно отдаёт мышь камере игры, даже если меню открыто. В распознанном SA:MP курсор удерживается через периодический `SetCursorMode`.
...
[I]По умолчанию в публичный репозиторий отправляются исходники, `.github/workflows`, профиль сборки Visual Studio и `README.md`. `context.md` и `README.txt` считаются локальными заметками и публикуются только по явному запросу; `AGENTS.md`, `.claude` и агентские правила остаются локально.[/I]
...
[*]Локально проект собирается Visual Studio 18 toolset `v145`; в GitHub Actions намеренно используется `v143`, потому что он доступен на `windows-latest`.
[/LIST]
